...
impl VersionDiff {
    ...
    pub fn matching_field(&self) -> Option<&str> {
        match self {
            Self::Latest { .. } |
            Self::Outdated { .. } => None,
            Self::Predownload { download_info, .. } => match download_info {
                DownloadOrDiff::Patch(SophonDiff { matching_field, .. }) |
                DownloadOrDiff::Download(SophonDownloadInfo { matching_field, .. }) => Some(matching_field)
            },
            Self::Diff { diff, .. } => Some(&diff.matching_field),
            Self::NotInstalled { download_info, .. } => Some(&download_info.matching_field)
        }
    }
}
...
impl VersionDiffExt for VersionDiff {
    ...
    /// genshin's impl
    fn download_to(&mut self, _folder: impl AsRef<Path>, _progress: impl Fn(u64, u64) + Send + 'static) -> Result<(), Self::Error> {
        Ok(())
    }
    ...
    fn install_to(&self, path: impl AsRef<Path>, thread_count: usize, updater: impl Fn(Self::Update) + Clone + Send + 'static) -> Result<(), Self::Error> {
        ...
        match self {
            ...
            Self::Predownload { download_info, current, .. } => {
